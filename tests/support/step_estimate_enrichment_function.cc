...
int main (int argc, char **argv)
{
  ...
  //solve 1d problem
  std::string
  rhs_expr = "-(exp(-(x*x)/(2*sigma*sigma))*(-2*sigma*sigma + x*x))/(2*sigma*sigma*sigma*sigma*sigma*sigma*pi)";
  std::string boundary_expr = "1.0/(2*pi*sigma*sigma)*exp(-(x*x)/(2*sigma*sigma))";
  ...
  exact_solution.initialize(Point<1>(),
                            sigma,
                            exact_soln_expr);
  ...
  for (unsigned int i = 0; i != interpolation_points.size(); ++i)
    {
      ...
      deallog << "Error in radial solution < 1e-3::"
              << (error < 1e-3)
              << std::endl;
    }
  ...
  SplineEnrichmentFunction<2> spline_approx(Point<2>(),
