  ...
  for (unsigned int i = 0; i != center.size(); ++i)
    {
      ...
      for (double x = left_bound[i]; x < right_bound[i]; x += 0.1)
      ...
      Step1::EstimateEnrichmentFunction problem_1d(
        Point<1>(center[i]), left_bound[i], right_bound[i], sigma,
        "1.0/(2*pi*sigma*sigma)*exp(-(x*x)/(2*sigma*sigma))", "0", 7);
      std::cout << "Solving with size,origin: " << left_bound[i] << ","
                << right_bound[i] << "," << center[i] << std::endl;
      problem_1d.run();

      // evaluate solution at values
      problem_1d.evaluate_at_x_values(interpolation_points, interpolation_values);
      double max_value =
        *max_element(interpolation_values.begin(), interpolation_values.end());

      // print points as output with sigma and coefficient
      std::stringstream name;
      name << "left_bound" << left_bound[i] << "right_bound" << right_bound[i]
           << "center" << center[i] << ".csv";
      ...
      for (unsigned int i = 0; i != interpolation_points.size(); ++i)
        {
          ...
          deallog << interpolation_points[i] << " " << interpolation_values[i]
                  << std::endl;
        }
      file.close();
    }
