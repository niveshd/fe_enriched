...
namespace Step1
{
    ...
    for (unsigned int i = 0; i != prm.n_enrichments; ++i)
      {
        Point<dim> p;
        prm.set_enrichment_point(p, i);
        vec_rhs[i].initialize(p, prm.sigma, prm.rhs_value_expr);
      }
   ...
   */
  template <int dim> void LaplaceProblem<dim>::make_enrichment_functions()
  {
    ...
    for (unsigned int i = 0; i < vec_predicates.size(); ++i)
      {
        ...
        double size = prm.predicate_radius * 4;
            ...
            if (prm.exact_soln_expr.size() == 0)
              {
                ...
                Point<dim> p;
                prm.set_enrichment_point(p, i);
                SplineEnrichmentFunction<dim> func(p, interpolation_points,
                                                   interpolation_values);
   ...
   */
  template <int dim> void LaplaceProblem<dim>::build_fe_space()
  {
  ...
  template <int dim> void LaplaceProblem<dim>::setup_system()
  {
    ...
    Point<dim> p;
    prm.set_enrichment_point(p, 0);
    boundary_value_func.initialize(p, prm.sigma, prm.boundary_value_expr);
  ...
  template <int dim> unsigned int LaplaceProblem<dim>::solve()
  {
...
// use this only when exact solution is known
  template <int dim> void LaplaceProblem<dim>::process_solution()
  {
  ...
  template <int dim> double LaplaceProblem<dim>::calculate_cond_num()
  {
    ...
    eigensolver.solve(system_matrix, eigenvalues, eigenfunctions,
                      eigenfunctions.size());
    ...
    pcout << "eigen values " << eig_large << " " << eig_small << std::endl;
