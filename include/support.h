...
template <int dim> struct EnrichmentPredicate
{
  EnrichmentPredicate(const Point<dim> origin, const double radius)
    : origin(origin), radius(radius) {}

  template <class Iterator> bool operator()(const Iterator &i) const
  {
...
template <int dim> class SplineEnrichmentFunction : public Function<dim>
{
public:
  ...
  SplineEnrichmentFunction(SplineEnrichmentFunction &&other)
    : Function<dim>(1), origin(other.origin),
